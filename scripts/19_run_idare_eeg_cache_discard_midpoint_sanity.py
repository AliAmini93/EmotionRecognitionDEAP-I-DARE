...
OUT_MD = ROOT / "docs" / "idare_eeg_cache_discard_midpoint_sanity.md"
OUT_JSON = ROOT / "docs" / "idare_eeg_cache_discard_midpoint_sanity.json"

DEFAULT_TASKS = ["valence", "arousal"]
DEFAULT_POLICIES = ["discard_midpoint"]
DEFAULT_SEEDS = [11, 13]
    ...
    for task in sorted({row["task"] for row in aggregate}):
        ...
        recommendations[task] = {
            ...
            "recommendation": (
                ...
                if clear
                else "This run evaluates the secondary discard-midpoint policy as a sanity comparison; do not treat it as a final LOSO result."
            ),
        }
...
def write_reports(payload: dict[str, Any]) -> None:
    ...
    lines: list[str] = []
    lines.append("# I-DARE EEG Cache-Based Discard-Midpoint Sanity Baseline\n")
    lines.append("This report was generated by `scripts/19_run_idare_eeg_cache_discard_midpoint_sanity.py`.\n\n")
    ...
    lines.append("- The previous focused policy baseline selected `midpoint_as_high` as the temporary main policy; this run checks the secondary `discard_midpoint` policy as a sanity comparison.\n")
    lines.append("- This discard-midpoint sanity baseline evaluates the secondary score-5 policy: `discard_midpoint` for both valence and arousal.\n\n")
            ...
            for fold in folds:
                for seed in args.seeds:
                    ...
                    if args.max_runs and run_count > args.max_runs:
                        break
                    print(
                        f"Running discard-midpoint sanity baseline: task={task}, policy={policy}, fold={fold.fold_id}, seed={seed}",
                        flush=True,
                    )
                    result = train_one(
                        task=task,
