...
echo "===== 3) generate representation/label-task diagnostic report ====="
"$PY" - <<'PY'
import json
import math
...
def label_audit_for_index(df, modality):
    ...
    for task in TASKS:
        ...
        exact_mid = np.isclose(raw.astype(float), 5.0, atol=1e-8) if raw_like else pd.Series([False] * len(raw), index=raw.index)
