...
OUT_MD = ROOT / "docs" / "idare_eeg_cache_focused_baseline.md"
OUT_JSON = ROOT / "docs" / "idare_eeg_cache_focused_baseline.json"

DEFAULT_TASKS = ["valence", "arousal"]
DEFAULT_POLICIES = ["midpoint_as_low", "midpoint_as_high"]
DEFAULT_SEEDS = [11, 13]
    ...
    for task in sorted({row["task"] for row in aggregate}):
        ...
        recommendations[task] = {
            ...
            "recommendation": (
                ...
                if clear
                else "Do not finalize from this run alone; keep both focused policies available."
            ),
        }
...
def write_reports(payload: dict[str, Any]) -> None:
    ...
    lines: list[str] = []
    lines.append("# I-DARE EEG Cache-Based Focused Baseline\n")
    lines.append("This report was generated by `scripts/17_run_idare_eeg_cache_focused_baseline.py`.\n\n")
    ...
    lines.append("- The previous multi-fold policy selection did not produce a final score-5 policy.\n")
    lines.append("- This focused baseline compares only the two surviving policies: `midpoint_as_low` and `midpoint_as_high`.\n\n")
            ...
            for fold in folds:
                for seed in args.seeds:
                    ...
                    if args.max_runs and run_count > args.max_runs:
                        break
                    print(
                        f"Running focused baseline: task={task}, policy={policy}, fold={fold.fold_id}, seed={seed}",
                        flush=True,
                    )
                    result = train_one(
                        task=task,
