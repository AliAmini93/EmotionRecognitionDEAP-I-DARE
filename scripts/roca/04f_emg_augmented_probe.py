...
OUT_MD = ROCA_DIR / "emg_augmented_probe_current.md"
OUT_JSON = ROCA_DIR / "emg_augmented_probe_current.json"
OUT_PRED_CSV = ROCA_DIR / "emg_augmented_probe_predictions_current.csv"
OUT_MAIN_CSV = ROCA_DIR / "emg_augmented_probe_main_metrics_current.csv"
OUT_SUBSET_CSV = ROCA_DIR / "emg_augmented_probe_subset_metrics_current.csv"
OUT_PER_SUBJECT_CSV = ROCA_DIR / "emg_augmented_probe_per_subject_metrics_current.csv"
...
MODELS = [
    "stimulus_only",
    "emg_expanded_ridge_residual_noaug",
    "emg_expanded_ridge_residual_aug",
    "emg_expanded_extratrees_residual_noaug",
    "emg_expanded_extratrees_residual_aug",
]

RIDGE_ALPHA = 100.0
AUG_COPIES = 2
AUG_NOISE_STD = 0.10
AUG_RANDOM_SEED = 20260513
EPS = 1e-8
...
def augment_scaled_features(x, y, rng):
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    ...
    for _ in range(AUG_COPIES):
        noise = rng.normal(loc=0.0, scale=AUG_NOISE_STD, size=x.shape)
        xs.append(x + noise)
        ys.append(y.copy())
...
def load_inputs():
    ...
    if x.shape[1] != len(columns):
        raise ValueError(f"Feature column mismatch: x_dim={x.shape[1]}, columns={len(columns)}")
    ...
    return x, idx, columns, stim, selected
...
def run_loso(x, idx, stim_pred):
    ...
    for target, score_col in TARGETS.items():
        for test_subject in subjects:
            ...
            x_train_scaled = apply_preprocessor(x_train_raw, prep)
            x_test_scaled = apply_preprocessor(x_test_raw, prep)
            ...
            valid = np.isfinite(y_train_dev)

            x_train_valid = x_train_scaled[valid]
            y_train_valid = y_train_dev[valid]

            rng = np.random.default_rng(AUG_RANDOM_SEED + int(test_subject) * 100 + (0 if target == "valence" else 1))
            x_aug, y_aug = augment_scaled_features(x_train_valid, y_train_valid, rng)

            # No augmentation Ridge
            beta_noaug = fit_ridge(x_train_valid, y_train_valid)
            pred_dev_ridge_noaug = predict_ridge(x_test_scaled, beta_noaug)

            # Augmentation Ridge
            beta_aug = fit_ridge(x_aug, y_aug)
            pred_dev_ridge_aug = predict_ridge(x_test_scaled, beta_aug)

            # No augmentation ExtraTrees
            et_noaug = make_extratrees(seed_offset=0)
            et_noaug.fit(x_train_valid, y_train_valid)
            pred_dev_et_noaug = et_noaug.predict(x_test_scaled)

            # Augmentation ExtraTrees
            et_aug = make_extratrees(seed_offset=1000)
            et_aug.fit(x_aug, y_aug)
            pred_dev_et_aug = et_aug.predict(x_test_scaled)
            ...
            specs = [
                ("emg_expanded_ridge_residual_noaug", pred_dev_ridge_noaug),
                ("emg_expanded_ridge_residual_aug", pred_dev_ridge_aug),
                ("emg_expanded_extratrees_residual_noaug", pred_dev_et_noaug),
                ("emg_expanded_extratrees_residual_aug", pred_dev_et_aug),
            ]

            for model_name, pred_dev in specs:
                pred_score = test_stim_mean + pred_dev
                r = common.copy()
                r["model"] = model_name
                r["y_pred_score_raw"] = pred_score
                r["y_pred_score_clipped"] = np.clip(pred_score, 1.0, 9.0)
                r["y_pred_deviation_raw"] = pred_dev
...
def write_outputs(pred, main_df, subset_df, per_subject_df, prep_df, feature_dim):
    ...
    summary = {
        "protocol": "strict LOSO expanded EMG train-only augmentation probe",
        "feature_dim": int(feature_dim),
        "augmentation": {
            "scope": "train subjects only",
            "space": "train-standardized feature space",
            "copies_per_train_row": AUG_COPIES,
            "gaussian_noise_std": AUG_NOISE_STD,
            "random_seed": AUG_RANDOM_SEED,
        },
        ...
        "preprocessing": {
            ...
            "standardization": "train mean/std",
    ...
    lines = []
    lines.append("# ROCA-I-DARE EMG Augmented Probe\n")
    lines.append("Strict LOSO. Train-only feature-space augmentation. Fixed config. No test tuning.\n")

    lines.append("## Configuration\n")
    lines.append(f"- feature_dim: `{feature_dim}`")
    lines.append(f"- aug_copies: `{AUG_COPIES}`")
    lines.append(f"- aug_noise_std: `{AUG_NOISE_STD}`")
...
def main():
    x, idx, columns, stim, selected = load_inputs()

    print("Running strict LOSO expanded EMG augmentation probe...")
    print(f"features shape: {x.shape}")
    print(f"feature columns: {len(columns)}")
    print(f"subjects: {idx['subject_id'].nunique()}")
    print(f"stimuli: {idx['stimulus_id'].nunique()}")
    print(f"Augmentation: copies={AUG_COPIES}, noise_std={AUG_NOISE_STD}")
    ...
    print()
    print("Main augmented EMG metrics:")
    print(main_lift[
        [
    ...
    print()
    print("Fold-safe hard subset augmented EMG metrics:")
    print(subset_lift[
        [
