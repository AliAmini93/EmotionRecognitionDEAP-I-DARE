...
AROUSAL_PRED = ROCA_DIR / "eeg_gaussian10_arousal_test_oracle_predictions_current.csv"
VALENCE_PRED = ROCA_DIR / "eeg_gaussian10_valence_test_oracle_predictions_current.csv"

AROUSAL_EEG_MODEL = "eeg_bc_residual_huber_norm_gaussian10_arousal_test_oracle"
VALENCE_EEG_MODEL = "eeg_bc_residual_huber_norm_gaussian10_valence_test_oracle"
BASELINE_MODEL = "stimulus_only"

OUT_MD = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_report_current.md"
OUT_JSON = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_report_current.json"
OUT_PRED = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_predictions_current.csv"
OUT_POOLED = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_pooled_metrics_current.csv"
OUT_SUBJECT = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_subject_metrics_current.csv"
OUT_SUBJECT_SUMMARY = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_subject_summary_current.csv"
OUT_COMPARISON = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_eeg_vs_stimulus_current.csv"
OUT_FAILURE = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_failure_ranking_current.csv"
OUT_CONFUSION = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_confusion_current.csv"
OUT_CLASS_REPORT = ROCA_DIR / "joint_valence_arousal_oracle_subject_macro_classification_report_current.csv"
...
def build_comparison(subject_metrics: pd.DataFrame, pooled_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for task in subject_metrics["task"].unique():
        eeg = subject_metrics[
            subject_metrics["model_group"].eq("eeg_gaussian10_oracle") & subject_metrics["task"].eq(task)
        ].copy()
    ...
    for task in pooled_metrics["task"].unique():
        eeg = pooled_metrics[
            pooled_metrics["model_group"].eq("eeg_gaussian10_oracle") & pooled_metrics["task"].eq(task)
        ]
        stim = pooled_metrics[
...
def main():
    ...
    eeg = make_joint_predictions(
        group_name="eeg_gaussian10_oracle",
