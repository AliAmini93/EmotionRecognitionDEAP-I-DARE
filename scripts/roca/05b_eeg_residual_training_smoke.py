...
OUT_MD = ROCA_DIR / "eeg_residual_training_smoke_current.md"
OUT_JSON = ROCA_DIR / "eeg_residual_training_smoke_current.json"
OUT_PRED_CSV = ROCA_DIR / "eeg_residual_training_smoke_predictions_current.csv"
OUT_MAIN_CSV = ROCA_DIR / "eeg_residual_training_smoke_main_metrics_current.csv"
OUT_SUBSET_CSV = ROCA_DIR / "eeg_residual_training_smoke_subset_metrics_current.csv"
OUT_FOLD_CSV = ROCA_DIR / "eeg_residual_training_smoke_fold_summary_current.csv"
...
MODEL_NAME = "eeg_bc_residual_huber_smoke"
EPS = 1e-8
...
def load_inputs(args):
    ...
    # The EEG cache index may already contain label columns. Drop them before
    # merging the official labels from idare_trial_index to avoid pandas
    # suffixes like valence_score_x / valence_score_y.
...
def predict_dev(model, x_np, batch_size, device):
    model.eval()
    preds = []
    with torch.no_grad():
...
def evaluate_dev_rmse(model, x_val, y_val, batch_size, device):
    pred = predict_dev(model, x_val, batch_size, device)
    err = pred - y_val
    return float(np.sqrt(np.mean(err * err)))


def train_for_epochs(x_train, y_train, args, device, epochs, x_val=None, y_val=None):
    model = make_model(device)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    loss_fn = nn.HuberLoss(delta=args.huber_delta)

    ds = EEGResidualDataset(x_train, y_train)
    loader = DataLoader(
        ds,
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        if x_val is not None and y_val is not None and len(x_val):
            val_rmse = evaluate_dev_rmse(model, x_val, y_val, args.batch_size, device)
            row["val_dev_rmse"] = safe_float(val_rmse)
    ...
    return model, best_epoch, best_val, history
...
def run_training(args):
    ...
    for fold_i, test_subject in enumerate(test_subjects, start=1):
        ...
        y_fit = loo_train_deviation(fit_df, score_col).astype(np.float32)
        y_val, _ = heldout_deviation(val_df, fit_df, score_col)
        y_val = y_val.astype(np.float32)

        _, best_epoch, best_val, hist = train_for_epochs(
            x_fit,
            y_fit,
            args,
            device,
        ...
        print(f"best_epoch={best_epoch} best_val_dev_rmse={best_val}")
        ...
        y_outer = loo_train_deviation(outer_train_df, score_col).astype(np.float32)
        final_model, _, _, final_hist = train_for_epochs(
            x_outer,
            y_outer,
            args,
            device,
        ...
        pred_dev = predict_dev(final_model, x_test, args.batch_size, device)
        ...
        fold_metric.update({
            ...
            "best_val_dev_rmse": safe_float(best_val),
    ...
    summary = {
        "protocol": "EEG residual training smoke",
        "target": args.target,
        "cache": args.cache,
        "model": MODEL_NAME,
        ...
        "loss": f"HuberLoss(delta={args.huber_delta})",
        "optimizer": {
            "name": "AdamW",
            "lr": args.lr,
        ...
        "notes": [
            "This is a smoke run, not the final full LOSO training.",
    ...
    lines = []
    lines.append("# ROCA-I-DARE EEG Residual Training Smoke\n")
    lines.append("This is a strict-LOSO smoke run, not final full training.\n")
    ...
    lines.append(f"- loss: `HuberLoss(delta={args.huber_delta})`")
    lines.append("")
    ...
    print("\nFold summary:")
    print(fold_df[[
        "test_subject", "best_epoch", "best_val_dev_rmse",
        "rmse", "dev_rmse", "dev_pearson", "final_train_last_loss"
