...
def parse_args():
    ...
    p.add_argument(
        "--oracle-test-checkpoint-selection",
        action="store_true",
        help=(
            "LEAKY/ORACLE diagnostic only: train on outer-train subjects and use "
            "the held-out test subject for early stopping/checkpoint selection. "
            "Invalid for clean LOSO reporting."
        ),
    )
    p.add_argument(
        "--configs",
...
def run_one_config(args, aug_name: str, aug_cfg: dict[str, Any], x_mmap, idx, stim_pred, selected, device):
    ...
    for fold_i, test_subject in enumerate(test_subjects, start=1):
        ...
        if args.oracle_test_checkpoint_selection:
            # In oracle/leaky mode we intentionally skip the clean inner validation
            # stage. The final model below will be trained on outer-train subjects
            # and selected using the held-out test subject.
            best_epoch = None
            best_val_scaled = None
            hist = []
            print(
                "[ORACLE-LEAKAGE MODE] Skipping clean inner validation. "
                "The held-out test subject will be used for checkpoint selection."
            )
        else:
        ...
        if args.oracle_test_checkpoint_selection:
            # LEAKY/ORACLE diagnostic:
            # Train on outer-train subjects and select the best checkpoint using
            # the held-out test subject itself. This is intentionally invalid for
            # clean reporting, but useful to estimate metric inflation.
            y_test_scaled_for_oracle = base.scale_target(true_dev, y_outer_mean, y_outer_std)

            final_model, best_epoch, best_val_scaled, final_hist = train_aug_with_early_stopping(
                x_train=x_outer,
                y_train_scaled=y_outer_scaled,
                x_val=x_test,
                y_val_scaled=y_test_scaled_for_oracle,
                args=args,
                device=device,
                aug_cfg=aug_cfg,
            )
            final_stage = "oracle_test_checkpoint_selection"
            print(
                f"[ORACLE-LEAKAGE MODE] selected best_epoch={best_epoch} "
                f"using test_subject={test_subject}; "
                f"oracle_test_rmse_scaled={best_val_scaled}"
            )
        else:
        ...
        for h in final_hist:
            ...
            hh.update({
                ...
                "oracle_test_checkpoint_selection": bool(args.oracle_test_checkpoint_selection),
            })
            history_rows.append(hh)
...
if __name__ == "__main__":
    raise SystemExit(
        "Do not run 05o_eeg_oracle_checkpoint_base.py directly. "
        "Use an oracle wrapper that redirects outputs to *_oracle_* files."
    )
