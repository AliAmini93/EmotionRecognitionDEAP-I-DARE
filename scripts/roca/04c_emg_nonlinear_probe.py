...
FEATURE_NPY = CACHE_DIR / "idare_emg_features_smoke.npy"
FEATURE_INDEX = CACHE_DIR / "idare_emg_feature_cache_index_smoke.csv"
...
OUT_MD = ROCA_DIR / "emg_nonlinear_probe_current.md"
OUT_JSON = ROCA_DIR / "emg_nonlinear_probe_current.json"
OUT_PRED_CSV = ROCA_DIR / "emg_nonlinear_probe_predictions_current.csv"
OUT_MAIN_CSV = ROCA_DIR / "emg_nonlinear_probe_main_metrics_current.csv"
OUT_SUBSET_CSV = ROCA_DIR / "emg_nonlinear_probe_subset_metrics_current.csv"
OUT_PER_SUBJECT_CSV = ROCA_DIR / "emg_nonlinear_probe_per_subject_metrics_current.csv"
...
MODELS = [
    "stimulus_only",
    "emg_direct_extratrees",
    "emg_residual_extratrees",
]
...
def load_inputs():
    missing = []
    for path in [FEATURE_NPY, FEATURE_INDEX, STIM_BASELINE_PRED, FOLD_SAFE_SELECTED]:
        if not path.exists():
    ...
    return x, idx, stim, selected


def make_model(seed_offset=0):
    params = dict(ET_PARAMS)
    params["random_state"] = int(ET_PARAMS["random_state"]) + int(seed_offset)
...
def run_loso(x, idx, stim_pred):
    ...
    for target, score_col in TARGETS.items():
        for test_subject in subjects:
            ...
            # Direct model: EMG -> SAM score
            y_train_score = train_df[score_col].to_numpy(dtype=float)
            direct = make_model(seed_offset=0)
            direct.fit(x_train, y_train_score)
            pred_direct = direct.predict(x_test)

            # Residual model: EMG -> deviation beyond train-only stimulus mean
            y_train_dev = train_deviation_target(train_df, score_col)
            valid = np.isfinite(y_train_dev)

            residual = make_model(seed_offset=1000)
            residual.fit(x_train[valid], y_train_dev[valid])
            pred_dev = residual.predict(x_test)
            ...
            d = common.copy()
            d["model"] = "emg_direct_extratrees"
            d["y_pred_score_raw"] = pred_direct
            d["y_pred_score_clipped"] = np.clip(pred_direct, 1.0, 9.0)
            d["y_pred_deviation_clipped"] = d["y_pred_score_clipped"] - d["train_stimulus_mean"]
            rows.append(d)

            r = common.copy()
            r["model"] = "emg_residual_extratrees"
            r["y_pred_score_raw"] = test_stim_mean + pred_dev
            r["y_pred_score_clipped"] = np.clip(r["y_pred_score_raw"], 1.0, 9.0)
    ...
    return pd.concat([pd.DataFrame(stim_rows), emg], ignore_index=True)
...
def write_outputs(pred, main_df, subset_df, per_subject_df):
    ...
    summary = {
        "protocol": "strict LOSO EMG nonlinear probe",
        "model": "ExtraTreesRegressor",
        "model_params": ET_PARAMS,
        "important_notes": [
            "Fixed hyperparameters; no test tuning.",
            "Train subjects only are used for fitting inside each LOSO fold.",
            "Hard subsets come from fold-safe Step 03 selection.",
            "Predictions are clipped to SAM range [1, 9] for score metrics.",
            "Residual model is the ROCA-relevant model.",
        ],
    ...
    lines = []
    lines.append("# ROCA-I-DARE EMG Nonlinear Probe\n")
    lines.append("Model: `ExtraTreesRegressor`. Strict LOSO. Fixed hyperparameters. No test tuning.\n")
...
def main():
    ...
    main_lift, subset_lift = write_outputs(pred, main_df, subset_df, per_subject_df)

    print()
    print("ROCA step 04c completed.")
    ...
    print()
    print("Main nonlinear EMG metrics:")
    print(main_lift[
        [
    ...
    print()
    print("Fold-safe hard subset nonlinear EMG metrics:")
    print(subset_lift[
        [
