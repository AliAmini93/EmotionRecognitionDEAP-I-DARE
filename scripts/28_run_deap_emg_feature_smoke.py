...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("# DEAP EMG Feature-Only Training Smoke")
    lines.append("")
    lines.append("This report was generated by `scripts/28_run_deap_emg_feature_smoke.py`.")
    lines.append("")
    lines.append("No raw DEAP `.dat` files were loaded inside training loops.")
    lines.append("")
    lines.append("## Inputs")
    lines.append("")
    ...
    lines.append("- This is the feature-level EMG-only path. Raw EMG-only should be a separate ablation after this path is stable.")
    lines.append("")
...
def main() -> int:
    ...
    print("[INFO] DEAP EMG feature-only training smoke")
